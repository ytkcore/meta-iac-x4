...
# Usage: ./tf.sh <env> <stack> <cmd> [args...]
# Examples:
#   ./tf.sh dev 00-network plan
#   ./tf.sh dev 10-security apply -auto-approve
#   VAULT_PROFILE=<profile> ./tf.sh dev 00-network plan
# -----------------------------------------------------------------------------
...
# Main
# -----------------------------------------------------------------------------
# Auto-init for commands that need backend
if [[ "${CMD}" =~ ^(plan|apply|destroy|refresh|output|show|validate)$ ]]; then
  "${TF_RUN[@]}" terraform -chdir="${STACK_DIR}" init -upgrade=false \
