...
data "aws_subnets" "public" {
  filter {
    name   = "vpc-id"
    values = [try(data.terraform_remote_state.network.outputs.vpc_id, "")]
  }
  filter {
    name   = "tag:Name"
    values = ["*pub*"]
...
data "aws_route53_zone" "public" {
  count        = contains(["public", "both"], var.dns_scope) && var.route53_zone_id == "" ? 1 : 0
...
locals {
  ...
  subnet_id = try(data.terraform_remote_state.network.outputs.subnet_ids[var.harbor_subnet_key], "")
  ...
  # Route53 Zone IDs
  network_zone_id = try(data.terraform_remote_state.network.outputs.route53_zone_id, "")

  # 최종적으로 사용할 Zone ID 결정 로직
  target_public_zone_id  = var.route53_zone_id != "" ? var.route53_zone_id : try(data.aws_route53_zone.public[0].zone_id, "")
  target_private_zone_id = local.network_zone_id != "" ? local.network_zone_id : try(data.aws_route53_zone.private[0].zone_id, "")
  harbor_alb_dns_name    = try(module.harbor.alb_dns_name, "")

  # Decoupling & Security optimization
...
# -----------------------------------------------------------------------------
# 5. Route53 Records (Split-Horizon)
# -----------------------------------------------------------------------------

resource "aws_route53_record" "harbor_public" {
  count           = contains(["public", "both"], var.dns_scope) && var.enable_route53_harbor_cname && local.target_public_zone_id != "" ? 1 : 0
  allow_overwrite = true

  zone_id = local.target_public_zone_id
...
resource "aws_route53_record" "harbor_private" {
  count           = contains(["private", "both"], var.dns_scope) && var.enable_route53_harbor_cname && local.target_private_zone_id != "" ? 1 : 0
  allow_overwrite = true

  zone_id = local.target_private_zone_id
  name    = local.final_hostname
  type    = "CNAME"
  ttl     = 300
  records = [local.harbor_alb_dns_name]
...
resource "null_resource" "seed_helm_charts" {
  ...
  depends_on = [module.harbor, aws_route53_record.harbor_public, aws_route53_record.harbor_private]
