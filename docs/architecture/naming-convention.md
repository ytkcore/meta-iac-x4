...
*   **Subnet**: dev-meta-snet-pub-a, dev-meta-snet-pub-c
...
*   **Subnet**: dev-meta-snet-k8s-cp-pri-a, dev-meta-snet-k8s-dp-pri-a
...
## 5. 주석 컨벤션 (Comment Convention)

코드의 가독성과 일관성을 유지하기 위해 Shell Script 및 HCL(.tf) 파일에서 동일한 주석 스타일을 사용합니다.

### 5.1 파일 헤더 (File Header)
파일의 최상단에는 파일의 목적, 설명, 작성자 등을 명시하며, `#`와 `=` (EQUAL SIGN)을 사용하여 구분합니다. 80 컬럼 길이를 권장합니다.
...
### 5.2 섹션 구분 (Section Header)
코드 내의 논리적인 블록(변수 선언, 리소스 그룹 등)을 구분할 때는 `#`와 `-` (HYPHEN-MINUS)를 사용하여 구분합니다.

```bash
# ------------------------------------------------------------------------------
# Section Title (e.g., Local Variables, AWS Provider)
# ------------------------------------------------------------------------------
```
