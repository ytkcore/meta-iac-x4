...
v0.4 (현재):                           TO-BE (고도화):
  ...
  [인증] 서비스별 개별                    [인증] Keycloak ── SSO (사용자 인증 전용)
  [시크릿] K8s Secret (평문)             [시크릿] Vault ── 동적 생성/회전/감사
  [워크로드] Node IAM (AWS 전용)         [워크로드] Vault AWS Secrets Engine (동적 STS)
```
...
│              ▼               ▼               ▼                   │
│           AWS API        GCP API         온프렘                  │
│       (Vault AWS SE    (Vault GCP SE   (Vault 동적              │
│        → STS)           → WIF)          시크릿)                 │
└──────────────────────────────────────────────────────────────────┘
```
...
[12-platform-identity-architecture.md](12-platform-identity-architecture.md)에서 정의한 4-Layer Stack 원안 대비, **L2(SPIRE)를 Vault가 흡수**하는 3-Layer로 조정한다.

| SPIRE 기능 | 현재 대체 | 현재 필요 여부 |
|-----------|-------------|-------------|
| Pod → CSP API 인증 | ✅ Vault AWS Secrets Engine (STS) | **필요 → 구현 완료** |
| Pod → 외부 서비스 인증 | ✅ Vault 동적 시크릿 | **필요** |
| Pod 간 mTLS (X.509) | ❌ 불가 (SPIRE 고유) | 불필요 (서비스 메시 미사용) |
| Attestation (Pod 검증) | △ Vault K8s Auth (SA 기반) | 불필요 (현재 규모) |

### 4.2 역할 분담: Keycloak (L1 SSO) + Vault (L2 Workload + Secrets)
...
Vault가 제공하는 것:
  워크로드 Identity (L2)   → K8s Auth → AWS SE → STS 임시 자격증명
  동적 시크릿              → DB, PKI, 자동 rotation
...
> 현재: 인프라 관리 Pod 위주, 서비스 간 통신 최소 → **Vault + Keycloak으로 충분**
...
변경:
  ...
  55-bootstrap (변경)    ← ALBC Helm App 추가 (Vault Agent Injector annotation)
                         ← Vault Helm App 추가 (AWS SE + K8s Auth 설정)
```
