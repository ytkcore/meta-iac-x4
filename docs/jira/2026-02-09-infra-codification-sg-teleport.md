...
Alertmanager/Prometheus **Internal Ingress 추가** 및 Prometheus Operator TLS 수정 포함.
...
- [x] **추가** `monitoring.yaml` — Prometheus/Alertmanager Internal Ingress 추가
- [x] **추가** `monitoring.yaml` — Prometheus Operator `tls.enabled: false` (CrashLoopBackOff 해결)
- [x] **추가** Teleport EC2에 SSM으로 11개 앱 직접 등록 (alertmanager, prometheus 포함)
...
| `gitops-apps/bootstrap/monitoring.yaml` | Prometheus/Alertmanager Ingress + Operator TLS 비활성화 |
...
| `701e61a` | Prometheus/Alertmanager Internal Ingress 추가 |
| `78a2b7b` | Prometheus Operator TLS 비활성화 (CrashLoopBackOff 해결) |
